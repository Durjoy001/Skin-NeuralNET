...
def vit_transforms(model_name: str, train: bool):
    ...
    # For training, use more aggressive augmentation to reduce overfitting
    if train:
...
def build_model(model_name, num_classes):
    ...
    if model_name == "swin_tiny":
        model = timm.create_model(
            ...
            pretrained=True,
            num_classes=0,
            drop_path_rate=0.3,  # Increased from 0.2 to reduce overfitting
        )
...
def run_for_model(model_name: str):
    ...
    optimizer = torch.optim.AdamW(
        [
            {"params": backbone_params,   "lr": 3e-5, "weight_decay": 0.1},  # Reduced LR, increased weight decay
            {"params": classifier_params, "lr": 3e-4, "weight_decay": 0.01},  # Added weight decay, reduced LR
        ]
    )

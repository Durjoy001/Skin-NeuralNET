...
def get_transforms(model_name: str, train: bool):
    ...
    if train:
        ...
        # Note: base transforms typically include Resize/Crop -> ToTensor -> Normalize
        # We need to apply augmentations before ToTensor, so we extract and reorder
        # For simplicity, we'll apply augmentations that work on PIL images before base
        # and RandomErasing (which works on tensors) after normalization
        aug_transforms = [
            transforms.RandomHorizontalFlip(0.5),
            transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1),  # Color augmentation (PIL)
            transforms.RandomAffine(degrees=10, translate=(0.1, 0.1), scale=(0.9, 1.1)),  # Geometric augmentation (PIL)
        ]
        # Apply PIL-based augmentations first, then base (which includes ToTensor + Normalize)
        # Then RandomErasing on the normalized tensor
        return transforms.Compose(aug_transforms + [base, transforms.RandomErasing(p=0.2, scale=(0.02, 0.1))])
    return base
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, labels in pbar:
        ...
        torch.nn.utils.clip_grad_norm_(model.parameters(), 0.5)  # Reduced from default for stronger regularization
        optimizer.step()
...
def run_for_model(model_name: str):
    ...
    optimizer = torch.optim.AdamW([
        {'params': backbone_params, 'lr': LR * 0.06, 'weight_decay': 0.1},  # Reduced LR, increased weight decay
        {'params': classifier_params, 'lr': LR * 0.6, 'weight_decay': 0.01}  # Reduced LR, added weight decay
    ])
